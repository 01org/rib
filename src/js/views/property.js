...
(function($, undefined) {
    ...
    $.widget('rib.propertyView', $.rib.baseView, {
        ...
        _modelUpdatedHandler: function(event, widget) {
            var matchedProps, eventName, okCallback, cancelCallback,
                value = event.newValue, node = event.node;
            widget = widget || this;
            ...
            if ((event['property'] == 'id' || event['node'].getType() == 'event')
                ...
                if (event['property'] == 'id' && value.trim() == '') {
                    ...
                    if (!$.isEmptyObject(matchedProps)) {
                        okCallback = function(e) {};
                        cancelCallback = function(e) {
                            // Restore to old value.
                            node.setProperty('id', event.oldValue);
                        ...
                        $.rib.confirm(
                            'All event handlers of this widget will stop \
                            working and can\'t be exported if the ID property \
                            is empty. Are you sure you want to set the ID \
                            property to empty?',
